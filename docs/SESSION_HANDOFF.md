...
- Observability: **Phases 0–4 done** — structured JSON logging, correlation IDs, secret redaction, hardened error filter, GCP Error Reporting (log-based), health probes (`/health/live`+`/health/ready`), Prometheus `/metrics`, OTel→Cloud Trace (guarded), and **alerting (Slack + `system_events`, deduped/suppressed, wired to the exception filter)**. Still TODO: Cloud Logging dashboards/retention (Phase 5), docs + Cloud Build→Cloud Run deploy + Cloud Monitoring alert policies (Phase 6). Live confirmation of metrics/traces/alert-policies in Cloud Monitoring/Trace needs a Cloud Run deploy.

...
- Observability: **Phases 0–2 done** — structured JSON logging, correlation IDs, secret redaction, hardened error filter, and GCP Error Reporting (log-based) via `nestjs-pino` (see MONITORING/DEPLOY PLAN above). Still TODO: metrics/health/trace (Phase 3), Slack alerts (Phase 4), dashboards/retention (Phase 5), docs + Cloud Run deploy (Phase 6).
